...
void CP_InitRndT(uint16_t seed) {  // id_us_a.asn // line 100+
	/*mov	ax, [seed]
		and ax, 0ffh
		mov[rndindex2], ax
		ret*/
...
int main()
{
	...
	int key_index = 0;
	unsigned char second_flag[24]{ 0 };
	...
	/* graphkdr.edu + kd_keen.c
	KEENJUMPUR1SPR			=	93	// gamestate.key_index = 0	(s_keenjumpup1.rightshapenum)
	KEENJUMPUL1SPR			=	100	// gamestate.key_index = 2	(s_keenjumpup1.leftshapenum)
	KEENDREAM3SPR			=	137	// gamestate.key_index = 4 & 6	(s_keendie3)
	KEENRUNR1SPR			=	73	// gamestate.key_index = 8 	(s_keenwalk1)
	KEENRUNR2SPR			=	74	// gamestate.key_index = 10 	(s_keenwalk2)
	KEENRUNR3SPR			=	75	// gamestate.key_index = 12	(s_keenwalk3)
	...
	*/
	...
	for (int i = 0; i < 24; i = i + 8) {
		rc2_cc_decrypt(&cx, arr2 + i, arr2 + i);
	}
